...
public class Controller {
	...
	public ArrayList<String> createOrderList() {
		...
		while (it.hasNext()) {
			...
			output = String.format("ID:%4d  Nama Staff: %-20s  Total: Rp.%5.3f         Status: %-10s\n", re.getID_pesanan(),
	...
	public ArrayList<String> createOrderItemlList(int orderID)
	{
		...
		while (it.hasNext()) {
			re = it.next();
			output = String.format("%-4d|%-24s|%5d|%5.3f", ++hitung, re.getNama_item(), re.getJumlah(),
					re.getTotal_harga());
			initData.add(output);
		}
		if (initData.isEmpty())
	...
	public ArrayList<String> createMenuList(int tampil_tipeMenu) {
		...
		while (it.hasNext()) {
			...
			String output = String.format("Menu ID:%4d  Nama: %-20s  Harga: %5.3f \t Tipe: %s", re.getID(), re.getNama(),
	...
	public String createPaymentList() {
		...
		while (it.hasNext()) {
			...
			if (re.getStatus_Kerja() == Staff.statusKerja_Selesai) {
				...
				output += String.format("ID Staff:%4d  Nama Staff:%-20s  Lama kerja:%5.3f Gaji:%5.3f\n", re.getID(),
		...
		output += "-------------------------------------------------------\n";
		output += String.format("Total Pembayaran:Rp.%.3f (%d)", total_pembayaran, no_staff);
		return output;
	}
}
